...
Enhanced with Near-Miss Learning System for pattern refinement
"""
...
# Import Near-Miss Learning System
try:
    from near_miss_learning_system import NearMissLearningSystem
    NEAR_MISS_AVAILABLE = True
except ImportError:
    print("Near-Miss Learning System not available - using standard prediction only")
...
class AILotteryPredictor:
    ...
    def _get_model_weights(self, game_type: str) -> Dict[str, float]:
        ...
        try:
            with psycopg2.connect(self.connection_string) as conn:
                with conn.cursor() as cur:
                    ...
                    if not performance_data:
                        # BALANCED ACCURACY WEIGHTS - Restored signal with controlled diversity
                        return {
                            'pattern_analysis': 0.18,
                            'frequency_analysis': 0.20,  # Restored for pattern recognition
                            'statistical_regression': 0.18,
                            'anomaly_detection': 0.16,
                            'hybrid_mathematical': 0.18,
                            'pure_random': 0.10  # REDUCED: Limited randomization to preserve signal
                        }
            ...
            # Return BALANCED ACCURACY default weights - signal with diversity
            return {
                'pattern_analysis': 0.18,
                'frequency_analysis': 0.20,  # Restored for pattern recognition
                'statistical_regression': 0.18,
