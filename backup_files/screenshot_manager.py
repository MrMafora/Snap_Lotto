...
PREVIEW_CACHE_SECONDS = 3600  # Cache previews for 1 hour
PREVIEW_NAVIGATION_TIMEOUT = 45000  # 45 seconds timeout for preview navigation
PREVIEW_MAX_WAIT_TIME = 60  # Maximum wait time in seconds for preview generation
PREVIEW_MAX_RETRIES = 5  # Allow more retries for preview-only operations
PREVIEW_RETRY_BACKOFF = [1, 2, 4, 8, 15]  # Exponential backoff in seconds
...
def generate_preview_image(url, retry_count=0, is_preview=False):
    """
    Generate a preview image of a website, optimized for the preview-website endpoint.
    This version has increased timeouts and refined retry logic with exponential backoff.
    
    Args:
        url (str): The URL to capture
        retry_count (int): Current retry attempt (for recursive retries)
        is_preview (bool): Whether this is being called for preview purposes
        
    Returns:
        tuple: (img_data, error_message) - image data as bytes and error message if any
    """
    import time
    
    # For preview requests, we use a higher retry limit than for regular screenshots
    max_retries = PREVIEW_MAX_RETRIES if is_preview else MAX_RETRIES
    
    # If we've exceeded max retries, give up
    if retry_count >= max_retries:
        logger.error(f"Maximum preview retry attempts ({max_retries}) exceeded for {url}")
        return None, f"Preview generation timed out. The website may be unavailable."
    
    try:
        # Get the backoff time for this retry attempt (use the last value if we're beyond the array)
        backoff_idx = min(retry_count, len(PREVIEW_RETRY_BACKOFF) - 1)
        backoff_time = PREVIEW_RETRY_BACKOFF[backoff_idx]
        
        logger.info(f"Generating preview for {url} - Attempt {retry_count + 1}/{max_retries}")
        
        # On retry attempts, add a delay with exponential backoff to avoid rate limiting
        if retry_count > 0:
            logger.info(f"Applying exponential backoff: waiting {backoff_time} seconds before retry {retry_count + 1}")
            time.sleep(backoff_time)
        
        # Choose a random user agent to vary our requests
        user_agent = random.choice(USER_AGENTS)
        
        # Use Playwright with optimized settings for preview generation
        with sync_playwright() as p:
            browser_type = p.chromium
            
            # Launch browser with minimal arguments for speed
            browser = browser_type.launch(
                headless=True,
                args=[
                    "--no-sandbox", 
                    "--disable-setuid-sandbox",
                    # Use low-resource mode for previews
                    "--disable-extensions",
            ...
            try:
                # Create context with different viewport, user agent per retry
                # This helps bypass anti-scraping measures
                viewport_width = 1280 if retry_count % 2 == 0 else 1440
                viewport_height = 900 if retry_count % 2 == 0 else 1024
                ...
                # Load cookies to bypass any login requirements
                if os.path.exists(cookie_file):
                    try:
                        with open(cookie_file, 'r') as f:
                            cookies = json.load(f)
                            context.add_cookies(cookies)
                            logger.info(f"Preview: Loaded {len(cookies)} cookies for {domain}")
                    except Exception as e:
                        logger.warning(f"Preview: Error loading cookies: {str(e)}")
                
                # Create page and navigate with extended timeout
                page = context.new_page()
                
                # Different request headers for each retry
                headers = {
                    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
                    "Accept-Language": "en-US,en;q=0.9",
                    "Cache-Control": "no-cache" if retry_count > 0 else "max-age=0",
                    "Pragma": "no-cache" if retry_count > 0 else "",
                    "Sec-Fetch-Dest": "document",
                    "Sec-Fetch-Mode": "navigate",
                    "Sec-Fetch-Site": "none",
                    "Sec-Fetch-User": "?1",
                ...
                if retry_count > 0:
                    # Add a referer on retry attempts to appear more like a user following links
                    headers["Referer"] = "https://www.google.com/search?q=south+africa+lottery+results"
                ...
                try:
                    # Navigate with increased timeout for preview purposes
                    logger.info(f"Preview: Navigating to {url} with {PREVIEW_NAVIGATION_TIMEOUT}ms timeout")
                    
                    # Vary wait_until strategy based on retry count for more resilience
                    wait_until = 'domcontentloaded' if retry_count % 2 == 0 else 'load'
                    
                    page.goto(url, wait_until=wait_until, timeout=PREVIEW_NAVIGATION_TIMEOUT)
                    
                    # Add a short wait for additional resources
                    page.wait_for_timeout(2000)
                    
                except Exception as e:
                    logger.warning(f"Preview: Navigation timeout/error for {url}: {str(e)}")
                    # Continue despite timeout - we might still get a usable partial page
                
                # Attempt to scroll slightly to trigger lazy-loaded content
                try:
                    if retry_count > 0:  # Only on retry attempts
                        logger.info("Attempting to scroll to trigger dynamic content loading")
                        page.evaluate("window.scrollBy(0, 200)")
                        page.wait_for_timeout(500)
                except Exception as scroll_err:
                    logger.debug(f"Non-critical error during scrolling: {str(scroll_err)}")
                
                # Capture screenshot regardless of whether navigation completed fully
                try:
                    buffer = page.screenshot(full_page=False, type='png')
                    
                    # Log success with timing information
                    elapsed = time.time() - start_time
                    logger.info(f"Preview: Successfully captured preview for {url} in {elapsed:.2f}s (Attempt {retry_count + 1})")
                    ...
                    logger.error(f"Preview: Error taking screenshot: {str(screenshot_err)}")
                    # This is a more specific error, so we'll propagate it for better diagnostics
                    raise screenshot_err
                
            except Exception as e:
                logger.error(f"Preview: Error generating preview (attempt {retry_count + 1}): {str(e)}")
                try:
                    browser.close()
                except:
                    pass
                
                # If it was a timeout or network error, retry with exponential backoff
                error_msg = str(e).lower()
                if any(err_type in error_msg for err_type in ["timeout", "net::", "network", "connection"]):
                    logger.info(f"Preview: Retrying after network/timeout error (attempt {retry_count + 1}/{max_retries})")
                    return generate_preview_image(url, retry_count + 1, is_preview=True)
                
                return None, f"Error generating preview: {str(e)}"
    
    except Exception as e:
        logger.error(f"Preview: Critical error generating preview: {str(e)}")
        
        # Still retry on critical errors if we have attempts left
        if retry_count < max_retries - 1:
            logger.info(f"Preview: Retrying after critical error (attempt {retry_count + 1}/{max_retries})")
            return generate_preview_image(url, retry_count + 1, is_preview=True)
