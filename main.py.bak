...
def view_html_content(screenshot_id):
    ...
    # Read the HTML file and modify it to hide popups
    try:
        with open(html_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
            
        # Add CSS to hide "Oops!" popups and error messages
        popup_remover_css = """
        <style>
        ...
        div:has(h1:contains("Oops")), 
        div:has(h2:contains("Oops")), 
        div:has(h3:contains("Oops")),
        div:has(p:contains("Something went wrong")),
        div:has(span:contains("network connectivity")),
        ...
        <script>
        // Script to remove error popups or dialogs when loaded
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('div, h1, h2, h3, p, span').forEach(el => {
                if(el.innerText && el.innerText.includes('Oops')) {
                    let parent = el.parentElement;
                    for(let i=0; i<5 && parent; i++) { // Try to find higher-level container
        ...
        # Insert the CSS in the <head> section
        if '<head>' in html_content:
            html_content = html_content.replace('<head>', '<head>' + popup_remover_css)
        else:
            # If no head tag, add it at the beginning of the body or the document
            if '<body>' in html_content:
                html_content = html_content.replace('<body>', '<body>' + popup_remover_css)
            else:
        ...
        # Fall back to sending the file directly if we can't modify it
