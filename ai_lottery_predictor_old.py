...
class AILotteryPredictor:
    ...
    def generate_ai_prediction(self, game_type: str, historical_data: Dict[str, Any], variation_seed: int = 1) -> LotteryPrediction:
        """Use AI to generate lottery number predictions with focused game-specific analysis"""
        try:
            ...
            - Pick {game_config['bonus_count']} bonus numbers from 1-{game_config['bonus_range']}
            ...
            - Generate {game_config['bonus_count']} bonus numbers (1-{game_config['bonus_range']}) if applicable
            ...
            {{
                "main_numbers": [num1, num2, num3, num4, num5, num6],
                "bonus_numbers": [bonus1, bonus2],
                "confidence_percentage": 75,
                "reasoning": "Detailed explanation of patterns found and why these numbers were selected. Focus on specific algorithmic behaviors or exploitable trends discovered in the data analysis."
            }}
            """
            
            # Use focused AI analysis with game-specific data
            response = self.client.models.generate_content(
                model="gemini-2.5-pro",
                contents=prediction_prompt,
                config=types.GenerateContentConfig(
                    response_mime_type="application/json",
                    temperature=0.7 + (variation_seed * 0.01),  # Slight variation based on seed
            ...
            if response.text:
                try:
                    ...
                    prediction = LotteryPrediction(
                        game_type=game_type,
                        predicted_numbers=prediction_data.get('main_numbers', []),
                        bonus_numbers=prediction_data.get('bonus_numbers', []),
                        confidence_score=prediction_data.get('confidence_percentage', 50) / 100.0,
                        prediction_method="AI_Gemini_Pattern_Analysis",
                        reasoning=f"Comprehensive analysis of {game_type} data: " + prediction_data.get('reasoning', 'Pattern-based prediction'),
