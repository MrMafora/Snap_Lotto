...
def process_batch(batch_rows, import_history_id, db):
...
def import_data_to_db(text_content, batch_size=5):
    """Import the parsed text data into the database in batches"""
    ...
    with app.app_context():
        ...
        for i in range(0, len(data_rows), batch_size):
            ...
            print(f"Processing batch {i//batch_size + 1} of {(len(data_rows) + batch_size - 1) // batch_size}")
            
            batch_success, batch_errors = process_batch(batch, import_history.id, db)
            success_count += batch_success
            error_count += batch_errors
            
            # Update import history
            if batch_success > 0:
                import_history.records_added += batch_success
            if batch_errors > 0:
                import_history.errors += batch_errors
            
            # Commit after each batch
            db.session.commit()
            print(f"Batch {i//batch_size + 1} completed: {batch_success} successful, {batch_errors} errors")
        
        # Update final import history
...
if __name__ == "__main__":
    ...
    # Import the data with smaller batch size
    result = import_data_to_db(text_content, batch_size=3)
