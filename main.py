...
def run_automation_step():
    ...
    if step == 'cleanup':
        try:
            # Import and run cleanup step
            import step1_cleanup
            result = step1_cleanup.run_cleanup()
            flash('Cleanup step completed successfully', 'success')
        except Exception as e:
            logger.error(f"Cleanup error: {e}")
            flash(f'Cleanup step failed: {str(e)}', 'error')
    elif step == 'capture':
        try:
            # Import and run capture step
            import step2_capture
            result = step2_capture.run_png_capture()
            flash('Screenshot capture completed successfully', 'success')
        except Exception as e:
            logger.error(f"Capture error: {e}")
            flash(f'Screenshot capture failed: {str(e)}', 'error')
    elif step == 'ai_process':
        try:
            # Import and run AI processing step
            import step3_ai_process
            result = step3_ai_process.run_ai_extraction()
            flash('AI processing completed successfully', 'success')
        except Exception as e:
            logger.error(f"AI processing error: {e}")
            flash(f'AI processing failed: {str(e)}', 'error')
    elif step == 'database':
        try:
            # Import and run database step
            import step4_database
            result = step4_database.run_database_update()
            flash('Database update completed successfully', 'success')
        except Exception as e:
...
# Initialize critical modules with error handling
...
def background_init():
    """Background initialization of non-critical modules"""
    try:
        init_lazy_modules()
...
def home():
    ...
    try:
        ...
        # Process all lottery results directly from raw SQL - include both formats
