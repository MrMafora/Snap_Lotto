def sync_all_screenshots(config_urls):
    """
    ...
    with app.app_context():
        try:
            ...
            try:
                ...
                # Store a flag in the database to clear the session data
                screenshot_cleanup_flag = SystemFlag.query.filter_by(name='clear_screenshot_session').first()
                if not screenshot_cleanup_flag:
                    screenshot_cleanup_flag = SystemFlag(name='clear_screenshot_session', value='1')
                    db.session.add(screenshot_cleanup_flag)
                else:
                    screenshot_cleanup_flag.value = '1'
                db.session.commit()
            except Exception as e:
                app.logger.error(f"Error setting screenshot cleanup flag: {str(e)}")